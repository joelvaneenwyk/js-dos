...
export function QuickSaveFrame() {
    ...
    return (
        <div class="quick-save-frame flex flex-col justify-center items-center -my-2">
            <CloudSaveButton />
...
function SaveButton(props: {
    ...
    return (
        <div
            class={"sidebar-button flex justify-center " + (props.disabled ? "opacity-50" : "")}
            onClick={onClick}
        >
            <DisketteIcon />
            {props.disabled !== true && (
                <div class={"text-badge " + props.textcolor + " " + props.bgcolor}>
                    {props.label}
                </div>
            )}
...
function LoadButton(props: { label: number | string; bgcolor: string; disabled?: boolean }) {
    const account = useSelector((state: State) => state.auth.account);
    const dispatch = useDispatch();
    const nonSerializableStore = useNonSerializableStore();
    function onClick() {
