...
export function EditorConf() {
    ...
    const { sockdriveEndpoint, sockdriveWssEndpoint } = useSelector(
        (state: State) =>
        ...
        if (bundleConfig) {
            ...
            for (const next of Object.values(sockdrives)) {
                if (next.owner.length > 0 && next.drive.length > 0) {
                    parts.push(
                        `imgmount ${next.num} sockdrive ${next.backend} ${next.owner} ${next.drive}`,
                    );
                }
            }
    ...
    return (
        <div class="editor-conf-frame flex flex-col flex-grow w-full gap-2">
                ...
                {dosboxconf.map(({ name, backend, contents }) => {
                    return (
                        <button
                            ...
                            onClick={() => {
                                dispatch(
                                    dosSlice.actions.dosBackend(
                                        backend === "dosboxX" ? "dosboxX" : "dosbox",
                                    ),
                                );
                                changeConfig(
                                    contents.replaceAll("{wss-makevm}", sockdriveWssEndpoint),
                                );
                            }}
                        >
                            {name}
                                ...
                                <div class="flex flex-col gap-2">
                                    ...
                                    3 0 1 1-6 0 3 3 0 0 1 6 0Z"
                                        ...
                                        <input
                                            ...
                                            type="text"
                                            value={sockdrives[num]?.owner ?? ""}
                                            onChange={(e) => {
                                                const newSockdrives = { ...sockdrives };
                                                newSockdrives[num] = {
                                                    num,
                                                    backend:
                                                        sockdrives[num]?.backend ??
                                                        sockdriveWssEndpoint,
