...
export function EditorFsFrame() {
    ...
    return (
        <div class="editor-fs-frame frame-root items-start px-4">
...
function toNode(fsNode: FsNode, path: string): NodeExt {
    ...
    if (fsNode.nodes !== null) {
        const children = fsNode.nodes.sort(nodeSorter).map((next) => toNode(next, fullPath));
