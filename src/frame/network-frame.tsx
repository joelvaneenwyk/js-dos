...
export function NetworkFrame() {
    ...
    const ipxLink =
        network.ipx === "connected"
            ? location.href + searchSeparator() + "ipx=1&server=" + network.server + "&room=" + room
            : null;
    ...
    return (
        <div class="network-frame frame-root items-start px-4 relative">
            ...
            {ipxLink !== null && (
                <div class="mt-4 text-sm alert alert-success shadow-lg flex flex-col">
                    ...
                    <div class="flex flex-row">
                        ...
                        <div class="contextmenu underline break-all" target="_blank">
                            {ipxLink}
                        </div>
                    </div>
                </div>
...
async function copyToClipBoard(text: string, t: (key: string) => string, dispatch: Dispatch) {
