...
export function SelectWindow() {
    ...
    if (useUrl) {
        return (
            <div class="select-window overflow-hidden flex-grow flex flex-col items-center justify-center px-8 py-8">
                <Load />
            </div>
        );
    }
    ...
    return (
        <div class="select-window overflow-hidden flex-grow flex flex-col items-center justify-center px-8">
            <div
...
function Load() {
    ...
    return (
        <>
            ...
            <div
                class="mt-4 text-center underline cursor-pointer hover:text-accent"
                onClick={() => loadBundle(url)}
            >
                {t("load")}
            </div>
            {error !== null && <div class="mt-8 text-center text-error">{error}</div>}
        </>
    );
}
...
function Upload() {
    ...
    async function onFileChange() {
        ...
        try {
            await loadBundleFromFile(file, store).catch((e) =>
    ...
    return (
        <div class="cursor-pointer" onClick={onUpload}>
            <svg
