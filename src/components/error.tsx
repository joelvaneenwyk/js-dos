...
export function Error(props: { error?: string | null; onSkip?: () => void }) {
    const t = useT();
    const error = props.error ?? "Unexpected error";

    return (
        <div class="flex-grow flex flex-col items-center justify-center px-8 m-auto">
            ...
            {props.onSkip && (
                <button class="mt-8 btn-primary" onClick={props.onSkip}>
                    {t("skip")}
                </button>
            )}
        </div>
