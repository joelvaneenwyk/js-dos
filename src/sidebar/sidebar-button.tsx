...
export function ImageRenderingButton(props: { class?: string }) {
    return (
        <SidebarButton
            class={props.class}
...
export function QuickSaveButton(props: { class?: string }) {
    const disabled = useSelector((state: State) => state.auth.account) === null && false;
    const action = disabled ? uiSlice.actions.modalLogin() : uiSlice.actions.frameQuickSave();
    return (
        <SidebarButton
...
export function FatDrivesButton(props: { class?: string }) {
    return (
        <SidebarButton
            class={props.class}
            frame="fat-drives"
...
export function DosboxConfButton(props: { class?: string }) {
    return (
        <SidebarButton class={props.class} frame="editor-conf" action={uiSlice.actions.frameConf()}>
            <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
...
export function FsButton(props: { class?: string }) {
    return (
        <SidebarButton class={props.class} frame="editor-fs" action={uiSlice.actions.frameFs()}>
            <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
...
export function CyclesButton() {
    ...
    return (
        <SidebarButton class="cycles" frame="stats" action={uiSlice.actions.frameStats()}>
            {cycles <= 0 && (
                <>
                    <span>~</span>
                    <sup>KC</sup>
            ...
            {cycles > 0 && cycles > 1000 && (
                <>
                    <span>{Math.round(cycles / 1000)}</span>
                    <sup>
                        <strong>K</strong>KC
                    </sup>
                </>
...
export function SettingsButton(props: { class?: string }) {
    return (
        <SidebarButton
            class={props.class}
...
export function SidebarButton(props: {
    ...
    return (
        <div
            class={"sidebar-button " + (hightlight ? "sidebar-highlight " : "") + props.class}
            onClick={onClick}
        >
            {props.children}
        </div>
